...
module.exports = {
  meta: {
    docs: {
      description:
        "require `let` or `const` instead of `var` when outside of try...catch",
  ...
  create: function(context) {
    ...
    var maxInOneLiner =
      context.options[1] !== undefined ? context.options[1] : 1;
    ...
    function check(node, allowOneLiners, maxInOneLiner) {
      ...
      var closeTokenLine = isBlockBody
        ? context.getTokenAfter(stmt).loc.start.line
        : context.getLastToken(stmt).loc.start.line;
      ...
      if (bodyStartLine - openTokenLine < 2) {
        context.report({
          ...
          fix: function(fixer) {
            return fixer.insertTextBefore(node.body.body[0], "\n");
          }
        });
      }
    }
